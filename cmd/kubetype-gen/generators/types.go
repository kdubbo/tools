...
import (
	...
	"github.com/kdubbo/tools/cmd/kubetype-gen/metadata"
)
...
// extracts values for Name and Package from "dubbostatus-override" in the comments
func statusOverrideFromComments(commentLines []string) (string, string, bool) {
	// ServiceEntry has a unique status type which includes addresses for auto allocated IPs, substitute DubboServiceEntryStatus
...
func (g *typesGenerator) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	for _, kubeType := range kubeTypes {
		...
		if name, packageName, found := statusOverrideFromComments(kubeType.RawType().CommentLines); found {
			localM["DubboStatus"] = c.Universe.Type(types.Name{Name: name, Package: packageName})
		}
